...
- **Appointment__c.Status__c canonical picklist** (2026-05-01): `Scheduled, Confirmed, Completed, Closed Won, Canceled, No Show`. American spelling (`Canceled`), `No Show` with **space** to match the 5 SOQL queries that read it. Frontend keeps lowercase kebab-case `apt.status`; `STATUS_FE_TO_SF` map in `server.js` translates on POST/PATCH. Mark as Restricted Picklist in SF Setup so future drift hard-fails. See `docs/memory/2026-05-01.md`.
